...
def get_dataset_deviation(trial_data, base_data):
    ...
    for i in range(0, 4800):
        ...
        new_record = get_vector_deviation(trial_data[i], base_data[base_index]).reshape(
            1, 128
        )
...
def pre_process(path, y_n):
    ...
    if y_n == "yes":
        ...
        data = preprocessing.scale(
            data, axis=1, with_mean=True, with_std=True, copy=True
        )
    else:
        data = preprocessing.scale(
            trial_data, axis=1, with_mean=True, with_std=True, copy=True
        )
    # convert 128 vector ---> 4*9*9 cube
    for vector in data:
        for band in range(0, 4):
            data_2D_temp = data_1Dto2D(
                vector[band * sub_vector_len : (band + 1) * sub_vector_len]
            )
